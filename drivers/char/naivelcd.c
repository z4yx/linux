...
static void nt35510_out32(const struct naivelcd_drvdata *drvdata,
                           u32 offset, u32 val)
{
...
static int nt35510_seek_point(const struct naivelcd_drvdata *drvdata, u32 x, u32 y){
...
static int naivelcd_open(struct inode *inode, struct file *file)
{
...
static int naivelcd_release(struct inode *inode, struct file *file)
{
    struct naivelcd_drvdata *drvdata = file->private_data;
    drvdata = (struct naivelcd_drvdata *)file->private_data;
    drvdata->isOpen = 0;
    return 0;
}
...
static loff_t naivelcd_llseek(struct file *file, loff_t offset, int whence)
{
  loff_t newpos;
  struct naivelcd_drvdata *drvdata = file->private_data;
  ...
  case 1:
    ...
    break;
  case 2:
    newpos = drvdata->xres * drvdata->yres * BYTES_PER_PIXEL + offset;
    break;
  default:
    return -EINVAL;
  }
  ...
  if ((newpos < 0) || (newpos >= drvdata->xres * drvdata->yres * BYTES_PER_PIXEL))
    return -EINVAL;
...
static struct file_operations fops =
{
  .owner = THIS_MODULE,
  .open = naivelcd_open,
  .write = naivelcd_write,
  .release = naivelcd_release,
  .llseek = naivelcd_llseek,
};

static int naivelcd_of_probe(struct platform_device *pdev)
{
	struct naivelcd_drvdata *drvdata;
  struct resource *res;
  ...
  if (!drvdata)
		return -ENOMEM;
  *drvdata = default_naivelcd_drvdata;
 ...
 #ifdef CONFIG_OF
 static const struct of_device_id naivelcd_match[] = {
 	{ .compatible = "lcd,naive", },
 	{ .compatible = "LCD,naive", },
 	{},
 };
 MODULE_DEVICE_TABLE(of, naivelcd_match);
 #endif /* CONFIG_OF */

 /*
  * Our device driver structure
  */
 static struct platform_driver naivelcd_driver = {
 	.probe		= naivelcd_of_probe,
 	.remove		= naivelcd_remove,
 	.driver	= {
 		.name	= DRV_NAME,
 		.of_match_table = of_match_ptr(naivelcd_match),
 	},
 };
...
module_init(naivelcd_init);
module_exit(naivelcd_exit);

 MODULE_DESCRIPTION("Naive LCD Driver");
